...
- [Sonub 코딩 가이드라인](#sonub-코딩-가이드라인)
  ...
  - [일반 코딩 표준](#일반-코딩-표준)
    - [API 함수 규칙](#api-함수-규칙)
      - [API 함수의 정의](#api-함수의-정의)
      - [API 함수 파라미터 규칙](#api-함수-파라미터-규칙)
      - [올바른 API 함수 예제](#올바른-api-함수-예제)
      - [잘못된 API 함수 예제 (절대 금지)](#잘못된-api-함수-예제-절대-금지)
      - [API 함수 작성 가이드](#api-함수-작성-가이드)
      - [내부 헬퍼 함수 예외](#내부-헬퍼-함수-예외)
      - [요약](#요약)
    - [에러 처리 표준](#에러-처리-표준)
...
- **✅ 필수**: API로 호출될 수 있는 모든 함수는 **배열 파라미터 하나**만 받아야 합니다
- **✅ 필수**: 배열 파라미터는 `$input` 또는 `$params`로 명명합니다
- **✅ 필수**: 함수 내부에서 배열 키로 필요한 값을 추출합니다
- **❌ 금지**: 여러 개의 개별 파라미터를 받는 함수는 API로 호출할 수 없습니다
- **❌ 금지**: 타입 힌트로 `int`, `string` 등 스칼라 타입을 직접 받는 것 금지 (API 호출 시 에러 발생)
...
/**
 * 사용자 목록 조회 함수 (API 호출 가능)
 *
 * @param array $input 입력 파라미터
 *   - int $input['per_page'] 페이지당 개수 (기본값: 20)
 *   - int $input['page'] 페이지 번호 (기본값: 1)
 * @return array 사용자 목록
 *
 * @example
 * // PHP에서 호출
 ...
 *
 * // JavaScript에서 API 호출
 * const result = await func('list_users', { per_page: 10, page: 2 });
 */
...
```php
<?php
// ❌ 잘못된 예제 1: 여러 개의 스칼라 파라미터
// JavaScript에서 func('request_friend', {me: 5, other: 10})로 호출 시
// "Argument #1 ($me) must be of type int, array given" 에러 발생!
function request_friend(int $me, int $other): void
{
    // ...
}

// ❌ 잘못된 예제 2: 타입 힌트가 있는 스칼라 파라미터
// JavaScript에서 API 호출이 불가능함
function get_user_info(int $user_id): array
{
    // ...
}

// ❌ 잘못된 예제 3: 파라미터가 전혀 없는 함수
// JavaScript에서 호출 시 에러 발생 가능
...
1. **파라미터 정의**: 배열 하나만 받도록 정의
   ```php
   function my_function(array $input = []): mixed
   ```

2. **파라미터 추출**: 함수 시작 부분에서 필요한 값 추출
   ```php
   $user_id = (int)($input['user_id'] ?? 0);
   $title = trim($input['title'] ?? '');
   ```

3. **검증**: 추출한 값 검증 및 에러 처리
   ```php
   if ($user_id <= 0) {
       error('invalid-user-id', '유효하지 않은 사용자 ID입니다');
   }
   ```

4. **PHPDoc 작성**: 배열 키와 타입을 명확히 문서화
   ```php
   /**
    * @param array $input 입력 파라미터
    *   - int $input['user_id'] 사용자 ID
    *   - string $input['title'] 제목
    */
   ```

5. **JavaScript 호출 예제**: PHPDoc에 JavaScript 호출 예제 포함
   ```php
   /**
    * @example
    * // JavaScript에서 API 호출
    * await func('my_function', { user_id: 123, title: 'Hello' });
    */
   ```

#### 내부 헬퍼 함수 예외

API로 노출되지 않는 **내부 헬퍼 함수**는 타입 힌트를 사용해도 됩니다:

```php
<?php
/**
 * 내부 헬퍼 함수: 두 사용자 ID를 정렬
 * (API로 호출되지 않음 - 내부 전용)
 *
...
단, 이러한 헬퍼 함수는 **절대로 JavaScript에서 직접 호출해서는 안 됩니다**.

#### 요약

- ✅ **API 함수**: `function my_func(array $input): mixed` - JavaScript에서 호출 가능
- ✅ **내부 헬퍼**: `function helper(int $a, int $b): mixed` - PHP 내부 전용
- ❌ **금지**: API로 사용할 함수에 스칼라 타입 파라미터 사용
