 ...
 *
 * // 프로필 구독
 * const profile = userProfileFirestoreStore.getProfile('user123');
 *
 * // Svelte 컴포넌트에서 사용
 * {#if profile}
...
class UserProfileFirestoreStore {
	...
	ensureSubscribed(uid: string | undefined): void {
		...
		if (!db) {
			console.error('[UserProfileFirestoreStore] ❌ Firebase Firestore가 초기화되지 않았습니다.');
			return;
		}
		...
		// 새로운 프로필 구독 시작
		// console.log(`[UserProfileFirestoreStore] 🆕 새 프로필 구독 시작: ${uid}`);
	 ...
	 * // $effect에서 구독 시작
	 * $effect(() => {
	 *   userProfileFirestoreStore.ensureSubscribed('user123');
	 * });
	 *
	 * // $derived에서 데이터 읽기
	 * const profile = $derived(userProfileFirestoreStore.getCachedProfile('user123'));
	 * ```
	 */
	getCachedProfile(uid: string | undefined): UserData | null {
		// uid가 없으면 null 반환
		if (!uid) {
			return null;
		}
	...
	/**
	 * 프로필 실시간 구독 시작 (Firestore)
	 *
	 * @param uid - 사용자 UID
	 */
	private subscribeToProfile(uid: string): void {
		// console.log(`[UserProfileFirestoreStore] ✅ 프로필 구독 시작: ${uid}`);
		// console.log(`[UserProfileFirestoreStore] 🔗 Firestore 경로: users/${uid}`);
		...
		unsubscribe = onSnapshot(
			...
			(snapshot) => {
				...
				if (!snapshot.exists()) {
					// 문서가 없는 경우
					// console.log(`[UserProfileFirestoreStore] ⚠️ 프로필 문서 없음: ${uid}`);
				...
				// console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
				// console.log(`[UserProfileFirestoreStore] 📥 프로필 데이터 수신: ${uid}`);
				...
				// console.log(`[UserProfileFirestoreStore] ✨ 캐시 업데이트 완료: ${uid}`);
				// console.log(`[UserProfileFirestoreStore] 📊 현재 캐시 크기: ${this.cache.size}`);
			},
			(error) => {
				// 데이터 로드 실패
				console.error('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
				console.error(`[UserProfileFirestoreStore] ❌ 프로필 로드 실패: ${uid}`);
		...
		// console.log(`[UserProfileFirestoreStore] 🎧 Firestore 리스너 등록 완료: ${uid}`);
	}
	...
	unsubscribeProfile(uid: string): void {
		...
		if (cached?.unsubscribe) {
			// console.log(`[UserProfileFirestoreStore] 🔌 프로필 구독 해제: ${uid}`);
			cached.unsubscribe();
	 ...
	 */
	clearAll(): void {
		// console.log('[UserProfileFirestoreStore] 🗑️ 전체 캐시 초기화');
	 ...
	 */
	debug(): void {
		// console.log('[UserProfileFirestoreStore] 📊 캐시 상태:');
...
/**
 * 전역 사용자 프로필 스토어 인스턴스 (Firestore)
 *
 * 애플리케이션 전체에서 하나의 인스턴스만 사용합니다.
 */
export const userProfileFirestoreStore = new UserProfileFirestoreStore();
