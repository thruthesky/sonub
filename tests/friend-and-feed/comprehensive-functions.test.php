...
// u2의 친구 ID 목록 가져오기
$friend_ids_result = get_friend_ids(['me' => $u2]);
$friend_ids_u2 = $friend_ids_result['friend_ids'];
...
// ============================================================================
// 테스트 6: get_hybrid_feed() 함수 (API 함수)
// ============================================================================
echo "테스트 6: get_hybrid_feed() 함수\n";
echo "----------------------------------------\n";

// 정상 조회
$hybrid_feed = get_hybrid_feed(['me' => $u2, 'limit' => 20, 'offset' => 0]);
test_assert(
    isset($hybrid_feed['feed']),
    "get_hybrid_feed는 'feed' 키를 포함한 배열을 반환해야 함"
);
test_assert(
    is_array($hybrid_feed['feed']),
    "'feed'는 배열이어야 함"
);
test_assert(
    count($hybrid_feed['feed']) >= 1,
    "u2의 하이브리드 피드에는 최소 1개 게시글이 있어야 함"
);

// 첫 항목 구조 확인
if (count($hybrid_feed['feed']) > 0) {
    $first = $hybrid_feed['feed'][0];
    test_assert(
...
// 잘못된 파라미터 테스트
test_throws(
    fn() => get_hybrid_feed(['me' => 0, 'limit' => 20]),
    'invalid-me',
    "me=0일 때 'invalid-me' 에러 발생"
);

test_throws(
    fn() => get_hybrid_feed(['me' => -5, 'limit' => 20]),
    'invalid-me',
    "me=-5일 때 'invalid-me' 에러 발생"
);

// LIMIT 범위 테스트 (자동 보정)
$hybrid_limit_0 = get_hybrid_feed(['me' => $u2, 'limit' => 0]);
test_assert(
    is_array($hybrid_limit_0['feed']),
    "limit=0일 때 자동으로 기본값(20)으로 보정되어야 함"
);

$hybrid_limit_200 = get_hybrid_feed(['me' => $u2, 'limit' => 200]);
test_assert(
    is_array($hybrid_limit_200['feed']),
    "limit=200일 때 자동으로 최대값(20)으로 제한되어야 함"
);

// OFFSET 테스트
$hybrid_offset = get_hybrid_feed(['me' => $u2, 'limit' => 1, 'offset' => 0]);
test_assert(
    count($hybrid_offset['feed']) <= 1,
...
test_assert(
    isset($finalized_u2['feed']),
    "finalize_feed_with_visibility는 'feed' 키를 포함한 배열 반환"
);

$feed_items_u2 = $finalized_u2['feed'];
...
$feed_items_u4 = $finalized_u4['feed'];
...
$feed_items_u1 = $finalized_u1['feed'];
...
$feed_items_u5 = $finalized_u5['feed'];
...
$scenario_feed = get_hybrid_feed(['me' => $u1, 'limit' => 50, 'offset' => 0]);
$scenario_post_ids = array_column($scenario_feed['feed'], 'post_id');
...
// 정렬 확인 (최신순)
if (count($scenario_feed['feed']) >= 2) {
    $is_sorted = true;
    for ($i = 0; $i < count($scenario_feed['feed']) - 1; $i++) {
        if ($scenario_feed['feed'][$i]['created_at'] < $scenario_feed['feed'][$i + 1]['created_at']) {
            $is_sorted = false;
            break;
        }
    }
