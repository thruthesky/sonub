<?php
/**
 * 게시판 카테고리 페이지 E2E 테스트
 *
 * '/post/categories' 페이지가 정상적으로 로드되는지 확인하는 테스트입니다.
 *
 * 실행 방법:
 * php tests/e2e/deploy-tests/categories.test.php
 */

echo "========================================\n";
echo "게시판 카테고리 페이지 E2E 테스트 시작\n";
echo "========================================\n\n";

// 테스트 URL
$test_url = 'https://local.sonub.com/post/categories';

echo "테스트 URL: $test_url\n\n";

try {
    // cURL을 사용하여 카테고리 페이지 요청
    $ch = curl_init();
    ...
    // 6. 카테고리 페이지 특정 콘텐츠 확인
    echo "✓ 테스트 6: 카테고리 페이지 콘텐츠 확인\n";

    // 카테고리 관련 텍스트 확인
    if (strpos($response, 'categories') !== false || strpos($response, '카테고리') !== false) {
        echo "  - 카테고리 관련 콘텐츠 확인됨\n";
    }

    // 게시판 목록 또는 카테고리 목록 확인
