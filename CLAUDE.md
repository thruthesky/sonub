...
**🔥🔥🔥 최강력 규칙: Sonub 프로젝트는 Docker 기반으로 동작합니다 🔥🔥🔥**

## Docker 컨테이너 구성

Sonub 프로젝트는 다음 3개의 Docker 컨테이너로 구성되어 있습니다:
...
**✅ 필수: PHP 명령어를 실행할 때는 반드시 `docker exec sonub-php` 명령을 사용하세요**

### PHP 관련 명령어 예시

```bash
# PHP 모듈 확인
docker exec sonub-php php -m

# PHP 버전 확인
docker exec sonub-php php -v

# PHP 설정 확인
docker exec sonub-php php -i

# GD 확장 확인
docker exec sonub-php php -m | grep -i gd

# PHP 스크립트 실행
docker exec sonub-php php /sonub/tests/db/db.test.php
```

## 테스트 실행

**✅ 필수: 테스트를 실행할 때는 반드시 `docker exec sonub-php` 명령을 사용하세요**
...
```bash
# Playwright는 호스트 환경에서 실행 (Docker 불필요)
npx playwright test tests/playwright/e2e/user-login.spec.ts
```

## 중요 사항

**⚠️⚠️⚠️ 주의: PHP 관련 작업 시 Docker 컨테이너를 반드시 사용하세요 ⚠️⚠️⚠️**

- **✅ 올바른 방법**: `docker exec sonub-php php -m`
- **❌ 잘못된 방법**: `php -m` (호스트 환경의 PHP를 사용하면 안 됨)

**이유**:
- 호스트 환경의 PHP와 Docker 컨테이너의 PHP 설정이 다를 수 있습니다
- 웹 서버(Nginx)는 Docker 컨테이너의 PHP-FPM을 사용합니다
- 테스트 및 디버깅 시 실제 운영 환경과 동일한 환경(Docker)에서 실행해야 합니다
...
**🔥🔥🔥 최강력 규칙: PHP 파일 수정 시에만 자동 배포합니다 🔥🔥🔥**

- [ ] **✅ PHP 파일 수정 시**: 개발자가 배포를 요청하지 않아도 무조건 자동으로 배포 실행
- [ ] **❌ 기타 파일만 수정 시**: 배포하지 않음 (Markdown, CSS, JavaScript, 테스트 파일만 수정한 경우)
- [ ] 개발자에게 배포 시작 통보 (PHP 파일 수정 시만): "🚀 PHP 파일 수정이 완료되어 자동으로 배포를 시작합니다."
- [ ] 프로젝트 루트 폴더에서 `./deploy.sh` 실행 (PHP 파일 수정 시만)
...
7. 테스트 작성
8. UTF-8 인코딩 확인
9. **🔥 PHP 파일 수정 시에만 자동 배포 시작 🔥**
10. `./deploy.sh` 실행 (PHP 파일 수정 시만, 내부적으로 모든 테스트 자동 실행 및 검증)
11. 배포 완료 또는 배포 생략 보고

**⚠️⚠️⚠️ 최강력 경고: "update" 명령은 모든 표준 가이드라인에 맞게 전면적으로 업데이트하며, PHP 파일 수정 시에만 자동으로 배포합니다 ⚠️⚠️⚠️**

---

# "문서 수정" 명령 워크플로우

**🔥🔥🔥 최강력 규칙: 개발자가 "문서", "문서 수정", "문서 업데이트" 요청 시 반드시 따라야 할 워크플로우 🔥🔥🔥**

개발자가 **"문서"**, **"문서 수정"**, **"문서 업데이트"** 등의 키워드로 요청하면, 이는 `docs/**/*.md` 문서를 찾아서 개발자가 원하는 내용을 적절한 위치에 추가하거나 수정하라는 의미입니다.

## "문서 수정" 명령의 목적

**핵심 목적**: 개발자가 요청하는 내용의 **문맥을 정확히 파악**하여, `docs/**/*.md` 내의 **가장 관련성 높은 문서**를 찾아서 **적절한 위치에 업데이트**하는 것입니다.

## "문서 수정" 워크플로우 단계

### 1단계: 문서 검색 및 분석 (필수)

**✅ 필수**: 개발자가 요청한 내용의 문맥을 정확히 파악하세요.

1. **요청 내용 분석**:
   - 개발자가 추가/수정하려는 내용이 무엇인가?
   - 어떤 주제나 카테고리에 속하는가?
   - 기술적 내용인가, 가이드라인인가, 규칙인가?
...
3. **문서 목록 확인**:
   ...
   - `docs/translation.md` - 다국어 번역 가이드라인
   - `docs/design-guideline.md` - 디자인 및 UI 가이드라인
   - `docs/api.md` - API 설계 및 사용 가이드라인
   - `docs/deployment.md` - 배포 프로세스 가이드라인
   - 기타 관련 문서

4. **문서 내용 읽기**:
   - 관련성이 높은 문서들을 **모두** 읽어서 내용을 파악
   - 각 문서의 구조, 섹션, 서브섹션을 확인
   - 개발자가 요청한 내용이 이미 문서에 존재하는지 확인

### 2단계: 개발자에게 통보 (필수)

**✅ 필수**: 작업을 시작하기 전에 다음 정보를 개발자에게 알립니다:

```
📋 문서 수정 계획:
- 대상 문서: docs/xxx.md, docs/yyy.md
- 추가/수정 위치: [섹션 이름] 섹션의 [서브섹션 이름] 부분
- 작업 내용: [요청한 내용을 어떻게 추가/수정할 것인지]
...
**🔥🔥🔥 최강력 규칙: 문서 내에서 가장 적절한 위치를 찾아야 합니다 🔥🔥🔥**

**적절한 위치 선정 기준**:

1. **주제별 그룹핑**:
   - 비슷한 주제끼리 모여 있는 섹션에 추가
   - 예: Docker 관련 내용 → "개발 환경" 또는 "테스트 환경" 섹션

2. **논리적 순서**:
   - 문서의 전체 흐름을 고려
   - 예: "기본 개념" → "사용 방법" → "고급 기능" → "문제 해결" 순서

3. **계층 구조**:
   - 적절한 헤딩 레벨 (##, ###, ####) 선택
   - 상위 섹션과 하위 섹션의 관계를 유지

4. **중복 방지**:
   - 동일하거나 비슷한 내용이 이미 존재하는지 확인
   - 존재하면 기존 내용을 **업데이트**하고, 없으면 **새로 추가**

**❌ 잘못된 위치 선정 예시**:
- 데이터베이스 관련 내용을 "디자인 가이드라인" 문서에 추가
- PHP 테스트 관련 내용을 "번역 가이드라인" 문서에 추가
- 섹션 순서를 무시하고 문서 맨 끝에 무작위로 추가

**✅ 올바른 위치 선정 예시**:
- Docker 관련 내용 → `docs/test.md`의 "테스트 환경" 섹션
- PHP 코딩 규칙 → `docs/coding-guideline.md`의 "PHP 코딩 가이드라인" 섹션
- 데이터베이스 쿼리 작성 → `docs/database.md`의 관련 섹션

### 4단계: 문서 업데이트 (필수)

**✅ 필수 작업**:

- [ ] 적절한 문서 파일 선택 (`docs/**/*.md`)
- [ ] 문서 내에서 가장 적절한 섹션/위치 선정
- [ ] 기존 내용과 중복되지 않도록 확인
- [ ] 문서의 전체 구조 및 흐름에 맞게 내용 추가/수정
- [ ] 마크다운 형식 및 스타일 일관성 유지
- [ ] 모든 내용은 **한국어**로 작성
- [ ] 코드 블록, 예제, 표 등 적절한 포맷 사용
...
**✅ 필수**: 문서 상단에 목차(ToC)가 있으면 **반드시 업데이트**하세요.

- [ ] 새로운 섹션/서브섹션을 추가한 경우 목차에 반영
- [ ] 기존 섹션 이름을 변경한 경우 목차에 반영
- [ ] 목차의 링크가 올바르게 작동하는지 확인
...
### 예시 1: 개발자가 "Docker 정보를 문서에 추가" 요청

**사용자 요청**: "Docker 컨테이너 정보를 문서에 추가해줘"

**AI 수행 작업**:
1. 요청 내용 분석: "Docker 컨테이너" → 개발 환경, 테스트 환경 관련
2. 관련 문서 검색: `docs/test.md` (테스트 환경), `CLAUDE.md` (개발 환경)
3. 문서 읽기: 두 문서의 구조 및 기존 내용 확인
4. 개발자에게 통보:
   ```
   📋 문서 수정 계획:
   - 대상 문서: CLAUDE.md, docs/test.md
   - 추가 위치: CLAUDE.md의 "개발 환경" 섹션, docs/test.md의 "테스트 환경" 섹션
   - 작업 내용: Docker 컨테이너 구성 및 사용 방법 추가
   ```
5. 적절한 위치에 Docker 정보 추가
6. 목차 업데이트 (필요한 경우)
7. UTF-8 인코딩 확인
8. 개발자에게 완료 보고

### 예시 2: 개발자가 "문서 수정" 요청 (구체적인 내용 포함)

**사용자 요청**: "문서 수정: PHP 테스트 실행 시 반드시 Docker를 사용해야 한다는 내용 추가"

**AI 수행 작업**:
1. 요청 내용 분석: "PHP 테스트 실행" + "Docker" → 테스트 관련 문서
2. 관련 문서 검색: `docs/test.md`
3. 문서 읽기: "테스트 실행 방법" 섹션 확인
4. 개발자에게 통보:
   ```
   📋 문서 수정 계획:
   - 대상 문서: docs/test.md
   - 수정 위치: "테스트 실행 방법" 섹션
   - 작업 내용: Docker 필수 사용 규칙 추가
   ```
5. "테스트 실행 방법" 섹션에 Docker 필수 사용 규칙 추가
6. 목차 확인 (필요시 업데이트)
7. UTF-8 인코딩 확인
8. 개발자에게 완료 보고

### 예시 3: 개발자가 "문서" 키워드만 사용

**사용자 요청**: "문서: 모든 함수는 PHPDoc으로 문서화해야 함"

**AI 수행 작업**:
1. 요청 내용 분석: "PHPDoc" + "함수 문서화" → 코딩 가이드라인 관련
2. 관련 문서 검색: `docs/coding-guideline.md`
3. 문서 읽기: "PHP 코딩 가이드라인" 섹션 확인
4. 개발자에게 통보:
   ```
   📋 문서 수정 계획:
   - 대상 문서: docs/coding-guideline.md
   - 추가 위치: "PHP 코딩 가이드라인" 섹션의 "주석 및 문서화" 부분
   - 작업 내용: PHPDoc 필수 사용 규칙 추가
   ```
5. 적절한 위치에 PHPDoc 규칙 추가
6. 목차 업데이트
7. UTF-8 인코딩 확인
8. 개발자에게 완료 보고
...
**⚠️⚠️⚠️ 최강력 경고: 문서 수정 시 반드시 문맥을 파악하여 정확한 문서의 적절한 위치에 추가하세요 ⚠️⚠️⚠️**

- **✅ 필수**: 요청 내용의 문맥을 정확히 파악
- **✅ 필수**: 관련성이 높은 문서를 **모두** 검색하고 읽기
- **✅ 필수**: 문서 내에서 가장 적절한 위치를 선정
- **✅ 필수**: 기존 내용과 중복되지 않도록 확인
- **✅ 필수**: 문서의 전체 구조 및 흐름 유지
- **✅ 필수**: 목차 업데이트 (필요한 경우)
- **✅ 필수**: UTF-8 인코딩 확인
- **❌ 금지**: 문서를 제대로 읽지 않고 무작위 위치에 추가
- **❌ 금지**: 잘못된 문서에 내용 추가
- **❌ 금지**: 문서 구조를 무시하고 추가
...
- [ ] **✅ PHP 파일 수정 시**: 개발자가 배포를 요청하지 않아도 무조건 자동으로 배포 실행
- [ ] **❌ CSS/JavaScript 파일만 수정 시**: 배포하지 않음
- [ ] 개발자에게 배포 시작 통보 (PHP 파일 수정 시만): "🚀 PHP 파일 수정이 완료되어 자동으로 배포를 시작합니다."
- [ ] 프로젝트 루트 폴더에서 `./deploy.sh` 실행 (PHP 파일 수정 시만)
- [ ] 배포 스크립트가 내부적으로 모든 테스트를 자동으로 실행하고 검증
- [ ] 테스트 통과 시 버전 업데이트, Git 커밋/푸시, SSH 배포 자동 실행
...
**CSS/JavaScript 파일만 수정 시 개발자에게 다음 정보를 보고**:
```
✅ 디자인 수정 완료
✅ 심플하고 단조로운 디자인 적용 완료
✅ Bootstrap 레이아웃 규칙 준수 완료
...
**🔥🔥🔥🔥🔥 초최강력 규칙: PHP 파일 수정 완료 후 개발자가 요청하지 않아도 반드시 자동으로 배포해야 합니다 🔥🔥🔥🔥🔥**

## 자동 배포 필수 규칙

**⚠️⚠️⚠️ 절대 준수: AI는 PHP 파일 수정 완료 후 개발자가 배포를 요청하지 않아도 워크플로우의 맨 마지막 단계에서 반드시 자동으로 배포해야 합니다 ⚠️⚠️⚠️**

- **✅ 절대 필수**: **PHP 파일 수정이 완료되면** 워크플로우의 맨 마지막 단계에서 즉시 자동으로 배포
- **✅ 절대 필수**: 개발자가 "배포"라고 명시적으로 요청하지 않아도 **PHP 파일 수정 완료 후 무조건 자동 배포**
- **✅ 절대 필수**: 배포 여부를 개발자에게 물어보지 않고 **즉시 배포 실행**
- **✅ 절대 필수**: PHP 파일 수정 워크플로우의 **맨 마지막 단계는 항상 자동 배포**
- **❌ 절대 금지**: "배포할까요?"라고 묻는 것은 절대 금지
- **❌ 절대 금지**: PHP 파일 수정 후 배포를 건너뛰거나 생략하는 것은 절대 금지
- **❌ 절대 금지**: 개발자가 배포를 요청할 때까지 기다리는 것은 절대 금지

**⚠️⚠️⚠️ 중요: 다음 경우에는 배포하지 마세요 ⚠️⚠️⚠️**

- **❌ 배포 제외**: Markdown 파일(`*.md`) 수정 또는 생성 시
- **❌ 배포 제외**: CSS 파일(`*.css`) 수정 또는 생성 시
- **❌ 배포 제외**: JavaScript 파일(`*.js`) 수정 또는 생성 시
- **❌ 배포 제외**: 테스트 파일(`*.test.php`, `*.spec.ts`) 수정 또는 생성 시
- **✅ 배포 실행**: **오직 PHP 파일(`*.php`)이 수정되었을 때만 배포**
...
**🔥🔥🔥 최강력 규칙: 오직 PHP 파일이 수정되었을 때만 자동으로 배포를 수행합니다 🔥🔥🔥**

**✅ 배포 실행 (PHP 파일 수정 시)**:

1. **PHP 코드 수정 완료 시**: `*.php` 파일 수정 후 → **즉시 자동 배포**
2. **PHP 파일 생성 완료 시**: 새로운 `*.php` 파일, 위젯, 페이지 생성 후 → **즉시 자동 배포**
3. **개발자가 "배포"라고 명시적으로 요청한 경우**: 당연히 배포
4. **개발자가 "~하고 배포"라고 요청한 경우**: PHP 파일 수정 여부와 무관하게 배포

**❌ 배포 제외 (PHP 파일 수정 없음)**:

1. **문서 업데이트 시**: `*.md` 문서 수정 또는 생성 시 → **배포하지 않음**
2. **CSS 수정 시**: `*.css` 파일 수정 또는 생성 시 → **배포하지 않음**
3. **JavaScript 수정 시**: `*.js` 파일 수정 또는 생성 시 → **배포하지 않음**
4. **테스트 작성 시**: `*.test.php`, `*.spec.ts` 파일 수정 또는 생성 시 → **배포하지 않음**
5. **설정 파일 수정 시**: JSON, YAML 등 설정 파일 수정 시 → **배포하지 않음** (PHP 파일 수정 제외)
...
**🔥🔥🔥 최강력 규칙: PHP 파일 수정 작업의 맨 마지막 단계는 항상 자동 배포입니다 🔥🔥🔥**

### 1단계: 배포 전 확인 (필수)

1. **PHP 파일 수정 여부 확인**:
   - [ ] PHP 파일(`*.php`) 수정 또는 생성 완료
   - [ ] 테스트 작성 완료 (테스트 실행은 배포 스크립트가 자동으로 수행)
   - [ ] UTF-8 인코딩 확인 완료
   - [ ] 다국어 번역 추가 완료 (필요한 경우)

2. **배포 제외 확인**:
   - [ ] Markdown 파일(`*.md`)만 수정한 경우 → 배포하지 않음
   - [ ] CSS 파일(`*.css`)만 수정한 경우 → 배포하지 않음
   - [ ] JavaScript 파일(`*.js`)만 수정한 경우 → 배포하지 않음
   - [ ] 테스트 파일(`*.test.php`, `*.spec.ts`)만 수정한 경우 → 배포하지 않음

3. **개발자에게 배포 시작 통보 (필수, PHP 파일 수정 시만)**:
   ```
   🚀 PHP 파일 수정이 완료되어 자동으로 배포를 시작합니다.
   📦 배포 스크립트: ./deploy.sh (내부적으로 모든 테스트 자동 실행 및 검증)
   ```

### 2단계: 배포 스크립트 실행 (필수, PHP 파일 수정 시만)

**✅ 필수**: PHP 파일이 수정되었을 때 개발자가 배포를 요청하지 않아도 무조건 자동으로 `./deploy.sh` 실행
...
**🔥🔥🔥 중요: PHP 파일 수정 시에만 자동으로 배포를 실행합니다 🔥🔥🔥**
...
### 예시 2: 개발자가 "PHP 코드 수정" 요청 (배포 요청 없음, 자동 배포)
...
1. PHP 코드 수정 작업 수행
2. UTF-8 인코딩 확인
3. **PHP 파일 수정 완료 후 자동으로 배포 시작 (개발자가 배포를 요청하지 않았지만 자동 실행)**
4. 개발자에게 배포 시작 통보: "🚀 PHP 파일 수정이 완료되어 자동으로 배포를 시작합니다."
5. `./deploy.sh` 실행 (테스트 자동 실행 및 검증 포함)
6. 개발자에게 "✅ 모든 작업 완료 ✅ 자동 배포 완료" 보고

### 예시 3: 개발자가 "문서 수정" 요청 (배포 요청 없음, 배포 안 함)
...
1. 문서 수정 작업 수행
2. UTF-8 인코딩 확인
3. **문서 파일만 수정했으므로 배포하지 않음**
4. 개발자에게 "✅ 문서 업데이트 완료 (PHP 파일 수정 없음, 배포 생략)" 보고

### 예시 4: 개발자가 "CSS 수정" 요청 (배포 요청 없음, 배포 안 함)

**사용자 요청**: "프로필 페이지 CSS 수정"

**AI 수행 작업**:
1. CSS 파일 수정 작업 수행
2. UTF-8 인코딩 확인
3. **CSS 파일만 수정했으므로 배포하지 않음**
4. 개발자에게 "✅ CSS 수정 완료 (PHP 파일 수정 없음, 배포 생략)" 보고

### 예시 5: 개발자가 "JavaScript 수정" 요청 (배포 요청 없음, 배포 안 함)

**사용자 요청**: "프로필 페이지 JavaScript 수정"

**AI 수행 작업**:
1. JavaScript 파일 수정 작업 수행
2. UTF-8 인코딩 확인
3. **JavaScript 파일만 수정했으므로 배포하지 않음**
4. 개발자에게 "✅ JavaScript 수정 완료 (PHP 파일 수정 없음, 배포 생략)" 보고

## "배포" 명령 자동 실행 규칙

**🔥🔥🔥🔥🔥 초최강력 규칙: PHP 파일 수정 완료 후 개발자의 명시적 요청 없이도 무조건 자동으로 배포를 수행합니다 🔥🔥🔥🔥🔥**

### 자동 배포가 실행되는 경우

**✅ 필수**: 다음과 같은 **경우**에 개발자가 "배포"라고 요청하지 않아도 **무조건 자동으로 배포**를 수행해야 합니다:

1. **PHP 코드 수정 작업 완료 시**: 개발자가 배포를 요청하지 않아도 자동 배포
2. **PHP 파일 생성 작업 완료 시**: 개발자가 배포를 요청하지 않아도 자동 배포
3. **개발자가 "배포"라고 명시적으로 요청한 경우**: 당연히 배포
4. **개발자가 "~하고 배포"라고 요청한 경우**: PHP 파일 수정 여부와 무관하게 배포

### 자동 배포가 실행되지 않는 경우

**❌ 배포 제외**: 다음과 같은 **경우**에는 개발자가 명시적으로 요청하지 않는 한 배포하지 않습니다:

1. **문서 수정 작업 완료 시**: `*.md` 파일만 수정한 경우 → 배포하지 않음
2. **CSS 수정 작업 완료 시**: `*.css` 파일만 수정한 경우 → 배포하지 않음
3. **JavaScript 수정 작업 완료 시**: `*.js` 파일만 수정한 경우 → 배포하지 않음
4. **테스트 작성 완료 시**: `*.test.php`, `*.spec.ts` 파일만 수정한 경우 → 배포하지 않음
5. **설정 파일 수정 시**: JSON, YAML 등 설정 파일만 수정한 경우 → 배포하지 않음

**❌ 절대 금지** (PHP 파일 수정 시):
- "배포할까요?"라고 묻는 것은 절대 금지
- PHP 파일 수정 후 배포를 건너뛰는 것은 절대 금지
- PHP 파일 수정 후 배포를 생략하는 것은 절대 금지
- 배포 여부를 개발자에게 확인하는 것은 절대 금지 (PHP 파일 수정 시)
- 개발자가 배포를 요청할 때까지 기다리는 것은 절대 금지 (PHP 파일 수정 시)
...
1. **PHP 파일 수정 완료 후**: 개발자가 요청하지 않아도 무조건 자동 배포
2. **배포 제외**: Markdown, CSS, JavaScript, 테스트 파일만 수정한 경우 배포하지 않음
3. **배포 스크립트 실행**: PHP 파일 수정 시 `./deploy.sh` 명령만 실행하면 됨
4. **자동 테스트 실행**: 배포 스크립트가 내부적으로 모든 테스트 자동 실행 및 검증
5. **자동 배포**: 테스트 통과 시 버전 업데이트, Git 커밋/푸시, SSH 배포 자동 실행
6. **완료 보고**: 개발자에게 배포 완료 통보

**✅ AI의 역할**:
- **PHP 파일 수정 시**: 작업 완료 → `./deploy.sh` 실행 → 결과 보고
- **기타 파일 수정 시**: 작업 완료 → 배포 생략 → 완료 보고
