...
**🔥🔥🔥 최강력 규칙: 개발자가 "배포" 요청 시 반드시 따라야 할 워크플로우 🔥🔥🔥**

## "배포" 명령이 실행하는 작업

개발자가 **"배포"**, **"~하고 배포"**, 또는 **테스트 완료 후 자동으로 배포**를 요청하면, AI는 **반드시** 다음 워크플로우를 따라야 합니다.
...
4. **SSH 배포**: 프로덕션 서버(`sonub@68.183.185.185`)에 SSH로 접속하여 `git pull` 실행
...
**🔥🔥🔥 최강력 규칙: 다음과 같은 경우 테스트 완료 후 자동으로 배포를 수행합니다 🔥🔥🔥**

- [ ] 개발자가 "~하고 배포"라고 명시적으로 요청한 경우
- [ ] 개발자가 "배포"라고 명시적으로 요청한 경우
- [ ] 개발자가 "테스트 후 배포" 또는 "테스트하고 배포"라고 요청한 경우

**⚠️⚠️⚠️ 최강력 경고: 배포는 모든 테스트가 통과한 후에만 수행해야 합니다 ⚠️⚠️⚠️**

- [ ] 테스트 실패 시 배포를 절대 수행하지 마세요
- [ ] 테스트가 없는 경우, 개발자에게 테스트 작성을 권장한 후 배포 여부를 확인하세요
