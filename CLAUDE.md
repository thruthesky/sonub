...
**🔥🔥🔥🔥🔥 초최강력 규칙: 모든 작업 완료 후 반드시 자동으로 배포해야 합니다 🔥🔥🔥🔥🔥**

## 자동 배포 필수 규칙

**⚠️⚠️⚠️ 절대 준수: AI는 모든 작업 완료 후 개발자가 요청하지 않아도 반드시 자동으로 배포해야 합니다 ⚠️⚠️⚠️**

- **✅ 절대 필수**: 코드 수정, 파일 생성, 문서 업데이트 등 **모든 작업이 완료되면 즉시 자동으로 배포**
- **✅ 절대 필수**: 개발자가 "배포"라고 명시적으로 요청하지 않아도 **작업 완료 후 자동 배포**
...
다음과 같은 모든 경우에 AI는 **반드시 자동으로** 배포를 수행해야 합니다:

1. **코드 수정 완료 시**: PHP, JavaScript, CSS 등 모든 코드 수정 후
2. **파일 생성 완료 시**: 새로운 파일, 위젯, 페이지 생성 후
3. **문서 업데이트 완료 시**: `*.md` 문서 수정 또는 생성 후
4. **테스트 완료 시**: 테스트 작성 및 실행 완료 후
5. **디자인 수정 완료 시**: CSS, HTML 구조 변경 후
6. **번역 추가 완료 시**: 다국어 번역 텍스트 추가 후
7. **설정 변경 완료 시**: 설정 파일 수정 후
8. **기타 모든 작업 완료 시**: 위에 나열되지 않은 모든 작업 완료 후

**✅ 필수**: 모든 개발 및 테스트 작업이 완료된 후 프로덕션 서버에 배포
**✅ 필수**: 배포 스크립트 `./deploy.sh`를 프로젝트 루트 폴더에서 실행
**✅ 필수**: 배포 스크립트는 버전 업데이트, Git 커밋/푸시, SSH를 통한 서버 업데이트를 자동으로 수행
...
**배포 스크립트가 수행하는 작업**:
1. **버전 업데이트**: `etc/app.version.php`의 `APP_VERSION` 상수를 현재 날짜-시간(`YYYY-MM-DD-HH-MM-SS`)으로 자동 업데이트
2. **Git 커밋**: 모든 변경사항을 "Commit for the release of the version: YYYY-MM-DD-HH-MM-SS" 메시지로 커밋
3. **Git 푸시**: 변경사항을 원격 저장소에 푸시
4. **SSH 배포**: 프로덕션 서버에 SSH로 접속하여 `git pull` 실행

### 3단계: 배포 결과 확인 (필수)

- [ ] 배포 스크립트가 성공적으로 실행되었는지 확인
- [ ] 에러 없이 "✅ 배포 완료!" 메시지가 출력되었는지 확인
- [ ] 배포 실패 시 에러 메시지를 개발자에게 보고
...
**AI 수행 작업**:
1. 모든 작업 완료 확인 (코드 수정, 테스트 통과, UTF-8 인코딩 확인)
2. 개발자에게 배포 시작 통보
3. 프로젝트 루트 폴더에서 `./deploy.sh` 실행
4. 배포 스크립트 실행 결과 확인
5. 개발자에게 배포 완료 보고
...
2. 모든 테스트 통과 확인
3. `./deploy.sh` 실행하여 배포
4. 개발자에게 테스트 및 배포 완료 보고

**사용자 요청**: "코드 수정 후 테스트하고 배포"
...
**🔥🔥🔥🔥🔥 초최강력 규칙: 모든 작업 완료 후 개발자의 요청 없이도 무조건 자동으로 배포를 수행합니다 🔥🔥🔥🔥🔥**

### 자동 배포가 실행되는 모든 경우

**✅ 필수**: 다음과 같은 **모든 경우**에 개발자가 "배포"라고 요청하지 않아도 **자동으로 배포**를 수행해야 합니다:

1. **코드 수정 작업 완료 시**: 자동 배포
2. **파일 생성 작업 완료 시**: 자동 배포
3. **문서 수정 작업 완료 시**: 자동 배포
4. **테스트 작성 및 실행 완료 시**: 자동 배포
5. **디자인 수정 작업 완료 시**: 자동 배포
6. **번역 추가 작업 완료 시**: 자동 배포
7. **설정 변경 작업 완료 시**: 자동 배포
8. **개발자가 "배포"라고 명시적으로 요청한 경우**: 자동 배포
...
- 배포를 건너뛰는 것은 절대 금지
- 배포를 생략하는 것은 절대 금지
- 배포 여부를 확인하는 것은 절대 금지

**⚠️⚠️⚠️ 최강력 경고: 배포는 모든 테스트가 통과한 후에만 수행해야 합니다 ⚠️⚠️⚠️**

- **✅ 필수**: 테스트가 있는 경우, 모든 테스트가 통과한 후에만 배포
- **✅ 필수**: 테스트 실패 시 배포를 절대 수행하지 마세요
- **✅ 권장**: 테스트가 없는 경우에도 즉시 배포 (테스트 작성은 선택 사항)
