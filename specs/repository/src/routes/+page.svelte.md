...
## 주요 기능
- 인증 상태에 따라 환영 카드 또는 로그인 유도 버튼을 노출합니다.
- 홈 화면 하단에 4개의 대시보드 카드(최근 사용자, 최근 오픈 채팅 메시지, 인기 오픈 채팅방, 최근 글 & 댓글)를 빈 Card UI로 표시하여 향후 데이터를 배치할 공간을 확보했습니다.
- "최근 사용자" 카드는 RTDB `/users`에서 `sort_recentWithPhoto` 기준으로 최신 5명의 `photoUrl`을 조회해 스택드 아바타 형태로 즉시 보여줍니다.
