---
title: delete-sample-users.ts
type: typescript
path: firebase/test/delete-sample-users.ts
status: active
version: 1.0.0
...
`firebase/test/delete-sample-users.ts` 스크립트는 Firestore `users` 컬렉션에 저장된 문서 중 `test: true` 필드가 설정된 임시 사용자를 삭제합니다. `tsx delete-sample-users.ts` 명령으로 수행하며, 같은 디렉터리의 `firebase-service-account-key.json` service account 키를 사용합니다.

## 실행 방법

```bash
cd firebase/test
npx tsx delete-sample-users.ts
```
...
- 2025-11-16: Codex Agent - test 필드 기반 삭제 로직으로 수정.
