---
title: delete-old-sample-users.ts
type: typescript
path: firebase/test/delete-old-sample-users.ts
status: active
version: 1.0.0
...
`firebase/test/delete-old-sample-users.ts` 스크립트는 Firestore `users` 컬렉션 전체를 조회하여 문서 ID가 `test_` 로 시작하는 사용자 문서를 삭제합니다. `tsx delete-old-sample-users.ts` 명령으로 실행하며, 동일 디렉터리에 있는 `firebase-service-account-key.json` service account 키로 Firebase Admin SDK를 초기화합니다.

## 실행 방법

```bash
cd firebase/test
npx tsx delete-old-sample-users.ts
```
...
- 2025-11-16: Codex Agent - 스크립트 작성 및 문서화.
