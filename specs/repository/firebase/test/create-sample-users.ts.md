---
title: create-sample-users.ts
type: typescript
path: firebase/test/create-sample-users.ts
status: active
version: 1.0.0
...
이 문서는 `firebase/test/create-sample-users.ts` 스크립트의 소스 코드와 실행 방법을 정리한 SED 문서입니다. Firebase Admin SDK를 사용하여 임시 테스트 사용자 100명을 Firestore `users` 컬렉션에 저장하며, 각 사용자 문서에는 `test: true` 플래그, picsum.photos 기반 랜덤 `photoUrl`, `gender`(M/F), 그리고 YYYYMMDD 숫자 형태의 `birthYearMonthDay`가 기록됩니다.

## 실행 방법

```bash
cd firebase/test
npx tsx create-sample-users.ts
```
...
- 2025-11-16: Codex Agent - 초기 스크립트 작성 및 SED 문서화.
- 2025-11-16: Codex Agent - test 플래그, 랜덤 사진/성별/생년월일 필드 추가.
- 2025-11-16: Codex Agent - 생년월일을 `birthYearMonthDay`(YYYYMMDD) 숫자 하나로 저장하도록 변경.
