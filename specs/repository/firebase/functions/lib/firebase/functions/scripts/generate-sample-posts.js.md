---
name: generate-sample-posts.js
description: generate-sample-posts Cloud Function
version: 1.0.0
type: firebase-function
category: cloud-function
original_path: firebase/functions/lib/firebase/functions/scripts/generate-sample-posts.js
---

# generate-sample-posts.js

## 개요

**파일 경로**: `firebase/functions/lib/firebase/functions/scripts/generate-sample-posts.js`
...
function initializeFirebase() {
    ...
    console.log(`📍 Credentials: ${credentialsPath || "Default Application Credentials"}`);
 ...
 */
function generatePostTitle(index) {
    const titles = [
        "안녕하세요",
        "좋은 정보 공유",
        "질문이 있습니다",
        ...
        for (let batch = 0; batch < Math.ceil(totalPosts / batchSize); batch++) {
            ...
            for (let i = startIndex; i < endIndex; i++) {
                ...
                const promise = postsRef
                    .push(postData)
                    .then((ref) => {
                    console.log(`✅ [${i + 1}/${totalPosts}] 글 생성 완료: ${ref.key}`);
                    createdCount++;
                    return { success: true, postId: ref.key };
                })
                    .catch((error) => {
                    console.error(`❌ [${i + 1}/${totalPosts}] 글 생성 실패: ${error.message}`);
...
// 스크립트 실행 시작
main();
//# sourceMappingURL=generate-sample-posts.js.map
```
