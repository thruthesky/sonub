...
- 사용자 정보 수정 및 프로필 사진 업로드 화면은 **`/my/profile`** 경로에서 제공됩니다.
- 해당 페이지는 `src/routes/my/profile/+page.svelte` (또는 동일 경로 하위 Svelte 파일)로 구현하며, 본 명세서의 요구사항을 모두 충족해야 합니다.
- 다른 경로나 다중 페이지로 분리하지 말고, `/my/profile`을 단일 소스로 유지하여 사용자 경험을 일관되게 관리합니다.

> **참고:** 로그인한 사용자 자신의 정보를 관리하는 페이지는 `/my/xxx` 경로 규칙을 따릅니다. 다른 사용자의 프로필을 조회하는 페이지는 `/user/profile/${uid}` 형식을 사용합니다.

## 프로필 수정 페이지 구현 요구사항

### 페이지 경로
- **URL**: `/my/profile`
- **파일**: `src/routes/my/profile/+page.svelte`
...
3. **생년월일 (birthYear, birthMonth, birthDay)**
   - 타입: 3개의 Selection Box (년/월/일 분리)
   - 필수 여부: 선택 사항
   - **birthYear 범위**:
     - 최소: 현재년도 - 70년 (예: 2025년 기준 1955년)
     - 최대: 현재년도 - 18년 (예: 2025년 기준 2007년)
     - 이유: 만 18세 이상만 가입 가능 (미성년자 제외)
   - **birthMonth 범위**: 1월 ~ 12월
   - **birthDay 범위**: 1일 ~ 31일
   - 저장 형식: YYYY-MM-DD (예: "1990-01-15")
   - 유효성 검증: 미래 날짜 방지

### 데이터 저장 형식

Firebase Realtime Database `/users/{uid}` 경로에 다음 형식으로 저장:
...
1. **인증 검증**
   - 비로그인 사용자는 `/user/login`으로 자동 리다이렉트
   - `authStore`를 사용하여 로그인 상태 확인

2. **데이터 로드**
   - 페이지 마운트 시 Firebase RTDB에서 기존 프로필 데이터 자동 로드
   - 신규 사용자의 경우 Firebase Auth의 displayName 사용

3. **저장 버튼**
   - 눈에 잘 띄는 디자인 (큰 크기, 강조 색상)
   - 전체 너비 버튼
   - 저장 중 로딩 상태 표시 ("저장 중...")
   - 비활성화 상태 처리

4. **피드백 메시지**
   - 성공 메시지: Alert 컴포넌트 사용 (녹색 또는 기본)
   - 에러 메시지: Alert 컴포넌트 사용 (빨간색, destructive)
   - 성공 메시지는 3초 후 자동 제거

5. **스타일링**
   - shadcn-svelte UI 컴포넌트 사용 (Card, Button, Alert)
   - TailwindCSS 유틸리티 클래스 사용
   - Light Mode Only
   - 모든 클릭 가능 요소에 `cursor-pointer` 적용
