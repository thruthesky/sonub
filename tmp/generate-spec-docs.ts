 ...
 *
 * 사용법:
 *   npx ts-node ./tmp/generate-spec-docs.ts
 *
 * 기능:
 * - src/routes/**/*.svelte 및 src/lib/components/**/*.svelte 파일을 읽어서
 * - ./specs/repository/src/ 경로에 SED 형식의 스펙 문서(.md)를 생성
 * - UTF-8 인코딩 보장
 * - YAML 헤더 포함
 *
 * 주의사항:
 * - 이미 존재하는 스펙 문서는 덮어쓰지 않음 (--force 옵션으로 강제 덮어쓰기 가능)
 * - 디렉토리가 없으면 자동으로 생성
 */
...
const CATEGORY_MAP: Record<string, string> = {
	...
	'src/lib/components/ui': 'ui-component',
	'src/lib/components': 'feature-component',
 ...
 * SED 스펙 문서 생성
 */
function generateSpecDoc(filePath: string, content: string): string {
 ...
 * 스펙 문서 파일 경로 생성
 */
function getSpecFilePath(sourceFilePath: string): string {
